...
function handleRows(ss,rows,bank,source){if(!rows.length)return respond({ok:false,error:'No rows'});const c=readConfig(ss),es=ss.getSheetByName(EXPENSES_TAB),is=ss.getSheetByName(INCOME_TAB);if(!es)return respond({ok:false,error:'Tab not found: '+EXPENSES_TAB});if(!is)return respond({ok:false,error:'Tab not found: '+INCOME_TAB});const ic=new Set([...DEFAULT_INCOME_CATS,...(c.customIncomeCats||[])]),sk=new Set(DEFAULT_SKIP_CATS);const er=ler(es,12),ir=ler(is,11);const eR=[],iR=[],eM=[],iM=[],bc={},dd=[];let s=0,d=0;for(const row of rows){const cat=row.category||'';if(sk.has(cat)){s++;continue}const base=gsr(bank,row.date,row.amount||row.net,row.vendor),bl=base.toLowerCase();bc[bl]=(bc[bl]||0)+1;const ref=bc[bl]>1?base+'-'+bc[bl]:base,rl=ref.toLowerCase();const netAmt=Math.abs(parseFloat(row.net)||parseFloat(row.amount)||0);const totAmt=Math.abs(parseFloat(row.amount)||parseFloat(row.net)||0);if(ic.has(cat)){if(ir.has(rl)){d++;dd.push({date:row.date||'',vendor:row.vendor||'',amount:parseFloat(row.amount||row.net)||0});continue}ir.add(rl);iR.push([row.date||'',row.vendor||'',row.description||'','',netAmt,cat]);iM.push([bank,ref])}else{if(er.has(rl)){d++;dd.push({date:row.date||'',vendor:row.vendor||'',amount:parseFloat(row.amount)||0});continue}er.add(rl);eR.push([row.date||'',row.vendor||'',row.description||'',netAmt,cat]);eM.push([bank,bank==='AMEX'?'Yes':'No',source||'Import',ref])}}if(eR.length){const st=gnr(es);es.getRange(st,2,eR.length,5).setValues(eR);es.getRange(st,9,eR.length,4).setValues(eM)}if(iR.length){const st=gnr(is);is.getRange(st,2,iR.length,6).setValues(iR);is.getRange(st,10,iR.length,2).setValues(iM)}return respond({ok:true,expenses:eR.length,income:iR.length,skipped:s,duplicates:d,duplicatesBlocked:d,duplicateDetails:dd,total:rows.length})}
